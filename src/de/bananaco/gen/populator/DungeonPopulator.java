...
public class DungeonPopulator extends BlockPopulator {
        ...
        if (density > 0.8) {
            ...
            for (int i = 0; i < roomCount; i++) {
                if (random.nextBoolean()) {
                    ...
                    int y = 48 + random.nextInt(48); // changed for skylands
    ...
    private void placeSpawner(Block block) {
        ...
        if(block.getState() instanceof CreatureSpawner)
        	((CreatureSpawner) block.getState()).setCreatureTypeByName((types[random.nextInt(types.length)].getName()));
        else
    ...
    private void placeChest(Block block) {
        ...
        // now spawn a random protector ontop of the chest!
        Location spawn = block.getRelative(BlockFace.UP).getLocation();
        EntityType[] types = new EntityType[]{
                EntityType.MAGMA_CUBE, EntityType.BLAZE, EntityType.SLIME, EntityType.CAVE_SPIDER, EntityType.CREEPER, EntityType.SPIDER
            };
        spawn.getWorld().spawnEntity(spawn, types[random.nextInt(types.length)]);
    }
